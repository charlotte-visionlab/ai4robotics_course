...
in order to learn to accurately predict handwritten digits (MNIST)
"""
...
class FullyConnectedNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        out_0 = torch.sigmoid(self.h_0(x))
        out_0 = self.drop(out_0)

        out_1 = torch.sigmoid(self.h_1(out_0))
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ###### Make Tensors Cuda Tensors If Cuda Is Available ######

        ###### Make Torch Tensors Variables ######
        data, target = Variable(data), Variable(target)

        ###### Inference Model and Calculate Loss ######
