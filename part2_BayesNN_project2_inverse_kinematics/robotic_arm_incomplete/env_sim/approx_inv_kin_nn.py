    ...
    if len(sprites) > 0 and num_steps_0 == 0 and num_steps_1 == 0:
        ...
        if torch.cuda.is_available():
            input_to_model = Variable(torch.from_numpy(
                np.asarray([(sprites[0][0] - 500.0 + 420) / 840, (sprites[0][1] - 500.0 + 420) / 840])).float().cuda())
        else:
            input_to_model = Variable(torch.from_numpy(
                np.asarray([(sprites[0][0] - 500.0 + 420) / 840, (sprites[0][1] - 500.0 + 420) / 840])).float())
        ...
        theta_0_true, theta_1_true = inv_kin_2arm(sprites[0][0] - 500, sprites[0][1] - 500, upperarm.scale, lowerarm.scale - 35)
