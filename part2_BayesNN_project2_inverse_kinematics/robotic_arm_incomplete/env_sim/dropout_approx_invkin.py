...
while True:
    ...
    if len(sprites) > 0 and num_steps_0 == 0 and num_steps_1 == 0 and mouse_state_bool:
        ...
        if torch.cuda.is_available():
            input_to_model = Variable(torch.from_numpy(np.asarray(
                [(sprites[0][0] - origin[0] + 420) / 840, (sprites[0][1] - origin[1] + 420) / 840])).float().cuda())
        else:
            input_to_model = Variable(torch.from_numpy(
                np.asarray([(sprites[0][0] - origin[0] + 420) / 840, (sprites[0][1] - origin[1] + 420) / 840])).float())
        ...
        with torch.no_grad():
            ...
            for iterator in range(sample_size_drop):
                ...
                theta_0_true, theta_1_true = inv_kin_2arm(sprites[0][0] - 500, sprites[0][1] - 500, upperarm.scale,
                                                          lowerarm.scale - 35)
                theta_0_true, theta_1_true = helpers.convert_normal_angle(theta_0_true, theta_1_true)
                print("x={} y={} theta_0={:.3f} theta_1={:.3f} theta_0_true={:.3f} theta_1_true={:.3f}".format(
                    sprites[0][0] - 500, sprites[0][1] - 500, theta_0, theta_1, theta_0_true, theta_1_true))
        ...
        if 'uncertainty_graphs' in locals():
            plt.clf()
        uncertainty_graphs = helpers.make_uncertainty_plots(sorted(lst_theta0[:-1]), sorted(lst_theta1[:-1]),
                                                            lst_theta0[-1], lst_theta1[-1])
        del (lst_theta0[:])
        del (lst_theta1[:])
        ...
        theta_0 = np.arctan2(theta_0_sin.data.item(), theta_0_cos.data.item())
        theta_1 = np.arctan2(theta_1_sin.data.item(), theta_1_cos.data.item())
        theta_0, theta_1 = helpers.convert_normal_angle(theta_0, theta_1)
